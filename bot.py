...
def filter_match_markets(df):
    """
    Filters a dataframe to keep only head-to-head match markets
    and remove outright / futures markets.
    """
    ...
    return df[df["title"].apply(is_match)].copy()
...
if __name__ == "__main__":
    ...
    # STEP 3 (filter outrights)
    tokens_df = filter_match_markets(tokens_df)

    tokens_df.to_csv("sports_tokens_filtered.csv", index=False)
